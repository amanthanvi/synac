...
Use "gpt-5.6-terra" as the served quality-ceiling classifier candidate and
"gpt-5.6-luna" as the low-cost served classifier candidate. Benchmark every
requested effort from `low` through `max`; never assume that a higher effort is
better. Do not count Luna and Terra as independent evidence families: they are
both named GPT-5.6 models from OpenAI, and no public evidence here establishes
disjoint base-model lineage. That is an inference from the model family naming,
not a claim about undisclosed weights.
...
| --- | --- | --- |
| Repository/runtime evidence | The production "content/tags.json" currently has **eight** tags; the synthetic-panel protocol plans a separate 1,500-entry, 11-tag study. The six requested Ollama tags resolve locally. A 110-case public-anchor agentic pilot now covers all ten requested Luna/Terra effort configurations. | Whether a given candidate accurately applies Tags to unseen SynAc Entries. The pilot is public, balance-disclosed, tool-using, and followed an invalidated first generation. |
...
The public pilot retains Terra `low` for ordinary-cost qualification and `xhigh`
for the hard-stratum ceiling; Terra `max` is currently dominated. That remains a
screening result, not a source-backed production-quality claim.
...
Use a distinct batch input file for each immutable "{model, snapshot-or-returned-model-id, effort, schema hash, prompt hash}" tuple. That keeps provenance and replay straightforward and prevents an effort comparison from being confounded by a different prompt or runtime. Batch is unsuitable for interactive reviewer feedback, but that is not this offline content workflow.

Illustration only—**not a forecast**: for 7,305 one-request-per-Entry
classifications averaging 600 input and 80 *total billed output* tokens, Batch
...
Pareto-prune the next served experiment to Luna `high`, Terra `low`, and Terra
`xhigh`. The others add no useful public-screen frontier. Run those three later
through one stateless Responses/Batch harness against fresh, non-public staged
reference cases, with identical prompt/schema, no tools, and archived usage.
This is a shortlist, not a production choice. Full artifacts and limitations
are in
[`experiments/tagging/served-model-bakeoff/results.md`](../../experiments/tagging/served-model-bakeoff/results.md).
