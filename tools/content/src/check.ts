...
if (!loaded.ok) {
  console.error(
    `content check failed: ${loaded.errors.length} schema error(s)\n`,
  );
...
const needsLabelEntries = new Set(
  dataset.senses
    .filter((sense) => sense.needsLabel)
...
console.log(`entries needing sense labels: ${needsLabelEntries.size}`);
...
if (emit) {
  ...
  console.log(
    `compiled dataset written to ${path.relative(repoRoot, outPath)}`,
  );
}
