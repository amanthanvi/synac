...
function integrationDatabaseNameAllowedForReset(dbName: string, serverIsLoopback: boolean): boolean {
  if (dbName.endsWith('_test')) return true;
  if (serverIsLoopback && (dbName === 'synac' || dbName === 'synac_staging')) return true;
  return false;
}
...
async function assertIntegrationDatabaseSafeForTruncate(prisma: PrismaClient): Promise<void> {
  ...
  const [meta] = await prisma.$queryRaw<Array<{ db: string; server_addr: string | null }>>(Prisma.sql`
  ...
  const serverAddr = meta.server_addr;
  const serverIsLoopback =
    serverAddr === null ||
    serverAddr === '127.0.0.1' ||
    serverAddr === '::1' ||
    serverAddr === '0:0:0:0:0:0:0:1';

  if (!integrationDatabaseNameAllowedForReset(meta.db, serverIsLoopback)) {
    throw new Error(
      `resetIntegrationDatabase refused: database "${meta.db}" is not an allowed integration test target (use localhost with synac/synac_staging, a *_test database name, or SYNAC_ALLOW_INTEGRATION_DB_RESET=1).`,
    );
  }
}
