...
export const browse = query({
  ...
  handler: async (ctx, args) => {
    ...
    const allTags = await ctx.db
      ...
      .take(500);
    const activeTag = args.tagSlug
      ? (allTags.find((tag) => tag.slug === args.tagSlug) ?? null)
      : null;

    const { start, end } = letterRange(args.letter);
    let rows = await ctx.db
      .query('entries')
      .withIndex('by_syncVersion_and_entryType_and_normalizedTitle', (q) =>
        q
          .eq('syncVersion', generation.version)
          .eq('entryType', args.entryType)
          .gte('normalizedTitle', start)
          .lt('normalizedTitle', end),
      )
      ...
      rows = rows.filter(
        (entry) =>
          ...
          entry.aliases.some((alias) =>
            alias.toLowerCase().includes(filterQuery),
          ),
      );
    }
    if (activeTag) {
    ...
    return {
      activeTag: activeTag
        ? { slug: activeTag.slug, name: activeTag.name }
        : null,
